...
MAXPILLS = 300   # cap knob (was hardcoded 300)
...
def play(seed):
    ...
    res = "stall"
    # TIME MODEL (pressure_rig_time only): each placement costs wall-time = T_LAT + FPR * rows_fallen.
    # A LOW board means long falls means MORE elapsed time per pill -- the owner's travel-time insight.
    elapsed = 0.0; T_LAT = 0.6; FPR = 0.35; TRATE = _C.get("trate", 0.0)
    ...
    for _ in range(MAXPILLS):
        ...
        cols_involved = [cc] if vertical else [cc, min(cc + 1, 7)]
        hmax = 0
    ...
    return {"elapsed_s": round(elapsed, 1), "seed": seed, "won": int(res == "clear"), "topout": int(res == "topout"),
