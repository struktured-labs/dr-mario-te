...
def fit_struktured_20260804(film_review_dir=FILM_REVIEW_DIR_DEFAULT, **kw):
    # VENDORED (2026-09-24): the fit re-derives from the owner's 2026-08-04 couch footage, which is not
    # published. With DRM_VENDOR=1, or when the footage dir is absent, load the fitted parameters from
    # the snapshot next to this file instead (bursty_owner_fit_20260804.json = vars() of this very fit;
    # identity vs the footage fit is asserted by experiments/cvx/tests_vendor_identity.py).
    if (os.environ.get("DRM_VENDOR") == "1" or not os.path.isdir(film_review_dir)) and not kw:
        snap = os.path.join(os.path.dirname(os.path.abspath(__file__)), "bursty_owner_fit_20260804.json")
        m = BurstyPressureModel.__new__(BurstyPressureModel)
        with open(snap) as f:
