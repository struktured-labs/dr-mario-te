...
R_VBONUS = 15
R_CVX = 16       # NONLINEAR (new): convex height penalty coefficient
R_KNEE = 17
...
def variant(name):
    ...
    if name == "r47":
    ...
    elif name.startswith("winsc"):
        # NEW-FEATURE arm: champion 'winner' + the planner's EARLY spawn-lane steering
        # term (spawn_col_height). FX's `spawn` only sees rows 0-3, i.e. it fires once
        # the spawn lane is ALREADY clustered; this one grades every non-virus cell in
        # cols 3/4 by height, so it steers off the lane from the first placement.
        # Viruses excluded: they are the TARGET, not clutter.  Name: winsc<WEIGHT>.
        w[R_VRDY] = 8.0; w[R_BURIED] = 48.0; w[R_RDYEXT] = 8.0
        w[R_SETUP] = 32.0; w[R_MATCHED] = 48.0
        ...
        # height 4->5 costs exactly what 13->14 costs -- but top-out is a CLIFF.
        # Adds  -CVX * max(0, maxh - KNEE)^2 .  Name: wincvx<CVX>_<KNEE>.
        w[R_VRDY] = 8.0; w[R_BURIED] = 48.0; w[R_RDYEXT] = 8.0
        w[R_SETUP] = 32.0; w[R_MATCHED] = 48.0
        _cvx, _knee = name[6:].split("_")
        w[R_CVX] = float(_cvx); w[R_KNEE] = float(_knee)
