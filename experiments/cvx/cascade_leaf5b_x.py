#!/usr/bin/env python3
"""STEER5b (POST-HOC, PREREG_STEER5b.md): cascade_leaf5_x + ONE more leaf term, w5[3] HSV:
  -W * #(viruses in cols 3..5 at row < 9) remaining in the leaf board ("clear the HIGH spawn-column viruses first";
  zero once they are gone, so it is active only while the count is > 0). Everything else is byte-for-byte the
  STEER5 module (the code of record for STEER5 stays cascade_leaf5_x.py, unedited).
...
def _x5(col, vir, fl, w5):
    if w5[0] == 0 and w5[1] == 0 and w5[2] == 0 and w5[3] == 0:
        return int64(0)
    bur = 0; acc = 0; rb = 0; hsv = 0
    ...
    for c in range(3, 6):
        ...
        for r in range(ROWS):
            ...
            if cc != 0:
                ...
                curcol = 0; curlen = 0
    return int64(-w5[0] * bur + w5[1] * acc - w5[2] * rb - w5[3] * hsv)
...
class Leaf5ReachDecider(ShapeReachDecider):
    ...
    def __init__(self, weights, flags, w5=(0, 0, 0, 0), **kw):
...
def selfcheck(n_games=2, seed0=36734):
    ...
    print(f"leaf5b selfcheck: {same}/{len(boards)} identical to the STEER4 baseline decider at w5 = 0")
