...
def classify(col,vir):
    ...
    for r in range(R):
        for c in range(C):
            ...
            if open_(r-1,c) or open_(r,c-1) or open_(r,c+1): out.append("drop")          # a clean drop reaches it
            elif (0<=c-1 and not occ[r,c-1]) or (c+1<C and not occ[r,c+1]): out.append("tuck")   # side cell empty but its column above is blocked
            else: out.append("buried")                                                    # every neighbour occupied: must DIG
    return out
with open(out,"w") as fh:
    for i in range(cnt):
        ...
        overh_plies=sum(1 for k,vc,cl in eg if all(x!='drop' for x in cl))
        anyover=sum(1 for k,vc,cl in eg if any(x!='drop' for x in cl))
        last=eg[-1]
        fh.write(json.dumps({"v":variant,"seed":s,"res":"clear" if r["won"] else ("topout" if r["topout"] else "stall"),
            ...
            "final_vleft":last[1],"final_drop":last[2].count("drop"),"final_tuck":last[2].count("tuck"),"final_buried":last[2].count("buried"),"eg_tuck_plies":sum(1 for k,vc,cl in eg if "tuck" in cl),"eg_buried_plies":sum(1 for k,vc,cl in eg if "buried" in cl),
