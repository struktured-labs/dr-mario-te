...
set -uo pipefail
SEED="${1:?usage: chain540_reach_tap_hsv_seed_build.sh <seed>}"
...
cp "$CP_QSF" NES.qsf
cp "$FW" copro_rom.hex
echo "PRE: RTL $(git rev-parse --short HEAD) fw $(md5sum copro_rom.hex | cut -c1-8) qsf=Childproof's NES.qsf.used; compiling seed $SEED $TAG  $(date -Is)"
