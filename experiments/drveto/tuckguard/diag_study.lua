...
local lf = io.open(OUT.."diag_study.log","w")
...
local function gates(tag)
  logf(string.format("  [%s] mode=%d players$0727=%d MATCH_ACTIVE=%d S2P_TTL=%d HOLD_ACTIVE=%d",
    tag, rd(0x46), rd(0x0727), rd(MATCH_ACTIVE), rd(S2P_TTL), rd(HOLD_ACTIVE)))
end
