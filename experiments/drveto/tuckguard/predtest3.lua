...
local TG_NEED, TG_OFF = 0x61B9, 0x61BA
...
emu.addEventCallback(function()
  ...
  if frame % 10 == 0 and not armed then
    ...
    if f then
      pubA, pubR = a, rt   -- FIX: W_TROW is TOP-relative (cart does TUCK_R2 = 15 - W_TROW, and TUCK_R2 is compared to $0386 which counts UP from the floor)
  ...
  if armed and frame - pend >= 6 then
    ...
    local c2, cr2, cneed, coff = rd(TUCK_C2), rd(TUCK_R2), rd(TG_NEED), rd(TG_OFF)
    if now ~= armed.board then
      ...
      cases = cases + 1
      logf(string.format("CASE %03d approach=%d final=%d trow_board=%d cart_TUCK_C2=%02X cart_TUCK_R2=%d cart_TG_NEED=%d cart_TG_OFF=%d verdict=%s board=%s",
        cases, armed.a, armed.f, armed.rt, c2, cr2, cneed, coff, (c2==0xFF) and "VETOED" or "ALLOWED", armed.board))
    end
