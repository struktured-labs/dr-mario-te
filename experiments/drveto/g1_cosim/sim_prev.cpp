...
struct Pub { int c, o, veto; bool search_phase; };
...
static void observe() {
  ...
  if (WE) {
    if (AB == 0x70F4) pending_cmd = DO & 0xF;
      ...
      if ((DO & 0xFF) != 0xFF)
        pubs.push_back({shadow_c, DO & 0xFF, shadow_b4, !tuck_started});
    } else if (AB == 0x61FF && (DO & 0xFF) == 1) done_seen = true;
  }
}
...
int main(int argc, char** argv) {
  ...
  const char* casefile = argc > 1 ? argv[1] : "g1cases.txt";
  ...
  for (int k = 0; k < n; k++) {
    ...
    int tcol = nes_read(0x5087), trow = nes_read(0x5088);
    ...
    char buf[64];
    for (auto& p : pubs) {
      snprintf(buf, sizeof buf, "%d:%d:%d:%s;", p.c, p.o, p.veto,
               p.search_phase ? "s" : "p");
      ps += buf;
    }
    printf("CASE %s final=%d,%d tuck=%d,%d done=%d clocks=%ld timeout=%d\n",
           name, rc, ro, tcol, trow, rdone, used, timeout ? 1 : 0);
    fflush(stdout);
  }
  fclose(f);
  delete t;
