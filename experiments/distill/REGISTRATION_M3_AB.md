...
- **B (candidate) = champion-const + the frozen distilled guard.** Guard
  ...
  **⚠ SUPERSEDED 2026-08-29: "both arms fork-free ⇒ ~12-25 core-s/pair" was
  true of the failure-rate endpoint and is FALSE of the decision-property
  primary that replaced it.** Recall is `P(g fires | the TEACHER fires)`, so
  the teacher's verdict must be known at the guard's own on-policy states —
  the tribunal has to RUN there. At ~95 forks x ~1.0 s per adjudicated
  trigger ply and 25.9 such plies/game, a 286-game labelled arm is
  **≈195 core-h**. Affordable (M1 was 270 core-h) but **not the near-free
  thing this line described.** See S1.7(b).
...
# S1.8 — ⚠⚠ M3 AND THE DAgger ROUND: SAME OPERATION, AND THEY MUST NOT SHARE DATA

Team-lead ruling, closing the question S1.7(b) opened. S1.7(b) observed that the
decision-property primary makes M3 structurally a labelling campaign over the
guard's on-policy trajectories — the DAgger shape. **The ruling is that they are
the same OPERATION and must never be the same DATA.**

**The hazard, stated so it cannot be rediscovered later:** DAgger's purpose is to
**IMPROVE** the student by training on its own state distribution; M3's purpose is
to **MEASURE** the student. **Run both over one labelled corpus and you have
trained on your test set** — the guard would be scored on the very states its
retraining consumed, and a GO would be uninterpretable.

⚠ **This would be nearly invisible in a write-up**, because both activities are
honestly described in the same words: *"labelling on-policy trajectories."* That
shared description is exactly what would let the circularity survive review.

## The rules, in force now

1. **M3 SPENDS THE OPERATION, NOT THE ROUND.** M3 labels on-policy trajectories
   **for EVALUATION ONLY. Those labels never train anything.**
2. **THE DAgger ROUND REMAINS UNSPENT AND CONTINGENT.** It is **not** decided
   now. It is decided **after M3 reports, on what M3 finds**:
   - student faithful on its own firing set ⇒ **no retraining needed, the option
     expires unused — a saving, not a loss**;
   - student unfaithful **specifically on its own state distribution** while
     faithful on the teacher's ⇒ that is the precise diagnosis DAgger exists to
     fix, and the round is worth spending.
3. **IF THE ROUND IS EVER SPENT IT REQUIRES FRESH ON-POLICY TRAJECTORIES** —
   generated by the **retrained** guard, which by DAgger's own logic visits a
   different state distribution anyway — **and a re-measurement on data disjoint
   from the training labels.** Written down now, while it costs nothing, rather
   than discovered when someone wants to reuse a corpus.
4. **THE "NO THIRD PASS" CLAUSE STANDS UNCHANGED.** M3 is not an attempt to
   improve the student, so it **does not consume a pass.**
