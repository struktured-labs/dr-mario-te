"""run_h14.py — H14 ENDPOINT runner (sealed-lineage copy of run_h12.py, 2026-08-21).

Deltas from run_h12.py, ALL additive:
  --level / --max-pills threaded to init_rig/play_one (defaults 11/300 keep
  the sealed behaviour bit-identical);
  BASE arm = H12Arm at the CERTIFIED config (tie_margin 0.5, topk 4, H=15,
  fork_samples 5, future=dist) — the champion being challenged;
  TRT arm  = H12Arm at the candidate config (--trt-tie-margin/--trt-topk/
  --trt-horizon/--trt-fork-samples).  label_mode=const collapses BOTH arms
  to the plain champion (identity gate).
---- original run_h12.py header ----
run_h12.py — H12 ENDPOINT ARM runner (sealed copy of run_oracle.py, 2026-08-15).
...
def _work(seed):
    ...
    t0 = time.monotonic()
    import h12_arm as H
    ab = H.H12Arm(label_mode=_W["label"], topk=_W["topk"],
    ...
    at = H.H12Arm(label_mode=_W["label"], topk=_W["trt_topk"],
                  horizon=_W["trt_horizon"], provenance=_W["provenance"],
                  future_mode="dist",
    ...
    if _W["provenance"]:
        rb["flip_log"] = ab.flip_log[:MAX_FLIPLOG]
...
def main():
    ...
    if a.exclude_file:
        excl = {int(x) for x in open(a.exclude_file).read().split()}
        seeds = [s for s in seeds if s not in excl]
    ...
    meta = {"model": a.model, "label": a.label, "future": a.future,
            "level": a.level, "max_pills": a.max_pills,
            ...
            "tie_margin": a.tie_margin, "arm_class": "H12Arm",
