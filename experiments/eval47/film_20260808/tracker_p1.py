#!/usr/bin/env python3
"""P1 (human) capsule tracker for the 20260808 dr_lulu capture, per match window.

Sibling of tracker_p2.py, and the same contract: every pure function comes from
`tracker.py` UNCHANGED. Only the crop grid, frames root and output prefix differ.
Nothing in tracker.py is modified. The per-frame loop below is copied from
tracker_p2.py so the two sides are tracked by identical logic -- the point of this
file is to measure the P1 side with the SAME instrument that produced P2's numbers,
so any difference in the control is a property of the footage, not of the code.

CROP GRID. The 60fps P1 clips are cut at (x=392, y=348), 440x704 --
refit_dr_lulu.py's P1_CROP_ORIGIN (documented, not derived). Crop-local grid is
    dict(P1, x0=432-392, y0=382.5-348) = dict(x0=40.0, y0=34.5, W=44.0, H=38.6)
VERIFIED by verify_p1_crop.py, which requires full-frame and cropped classification
to agree on sampled frames AND requires a grid shifted +8px / +22px to DISAGREE
(an equivalent mutant that moves crop and grid together proves nothing).

    tracker_p1.py <mid> <window_start_seconds>      e.g. tracker_p1.py m3 555
"""
...
FRAMES_60 = os.path.join(SESSION, "p1_60fps")
...
def load_frames(frames_root, n_frames):
    rows_idx, cols_idx = T.build_patch_index(P1_CROP)
...
def main():
    ...
    rows, paths = track(frames_root, n, window_start_abs, f"p1/{mid}")
    T.write_csv(rows, os.path.join(EVENTS_ROOT, f"p1_{mid}.csv"))
    T.write_paths_jsonl(paths, os.path.join(EVENTS_ROOT, f"p1_{mid}_paths.jsonl"))
    print(f"[p1/{mid}] wrote {len(rows)} pills / {len(paths)} path rows to {EVENTS_ROOT}",
          file=sys.stderr)
