...
def parse_leafeval(path):
    ...
    for line in body.splitlines():
        ...
        if m:
            ...
            continue
        # DRHSV form: the 15-bit two's complement accumulator, sign-extended into the 16-bit combine
        m = re.fullmatch(r"\+\s*\{\s*matched60_p\s*\[\s*14\s*\]\s*,\s*matched60_p\s*\}", line)
        if m:
            matched_in_combine = True; hsv_combine = True
            continue
    ...
    if matched_in_combine:
        ...
        # DRHSV: single accumulate site "matched60 <= matched60 + (cover ? 15'd48 : 15'd0) - (hsv ? 15'd512 : 15'd0)".
        # Only the COVER weight is parsed; the HSV rule (rows < 9, cols 3..5, -512) is SPEC-FIXED in the reference
        # (pyleaf_hsv / gate.py) so a wrong bound, column set or sign in the RTL FAILS the co-sim instead of parsing
        # into agreement.
        acc += [int(x) for x in re.findall(r"matched60\s*<=\s*matched60\s*\+\s*\(\s*\(curcol[^?]*\?\s*15'd(\d+)\s*:\s*15'd0\s*\)", src)]
