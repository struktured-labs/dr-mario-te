...
- runs the FULL 23-variant suite (incl. isolation vectors — a delta that miscounts
  one term fails exactly one vector and self-attributes)
- NOTE the RTL mailbox o4 is `variant XOR 2` (empirically recovered from the pinned
  node corpus, map (2,3,0,1); independently confirmed by fast_rtl_x's own
  `_VAR_OF_O4 = [2,3,0,1]`, which is self-inverse — the gate handles this;
  candidates never see o4)
...
23 weight variants per level-1 AND pairs run: shipped named sets (r47, winner,
vrdy12, weekend_burial, combined, cross8, parsed_rtl), 12 single-term isolation
vectors (any term miscount fails exactly one, so failures self-attribute),
flag-off isolations, and 2 wrap-stress vectors (multi-wrap both signs).

## Numba hazards (cost delta-eval a debugging cycle — 2026-07-30)

- numba bakes module-level constants (array sizes/layout) into compiled code but
  does NOT include them in the on-disk cache key. A stale `__pycache__` can serve
  functions compiled against an OLD layout — observed as `free(): invalid size`
  heap corruption. After changing any module-level constant a kernel closes over:
  `rm -rf __pycache__` in combo_term (and wherever the candidate lives).
- `NUMBA_BOUNDSCHECK=1` caught a genuine out-of-bounds write in a candidate
  harness. Worth one boundschecked pass of your candidate before gating
  (set the env var BEFORE the first import; it forces recompilation, so keep it
  out of routine runs).
