...
# ---------------------------------------------------------------- reference
# DRHSV (spec-fixed, NOT parsed from the RTL): -HSV_W per virus in cols 3..5 at row < 9, added INSIDE the signed-16
# combine. The wrap is modular, so s16(_eval_rtl(...) - HSV_W*n) == the RTL's single in-combine wrap exactly, and the
# blessed _eval_rtl source stays untouched.
HSV_W, HSV_ROWS, HSV_COLS = 512, 9, (3, 4, 5)
HSV_ON = False


def hsv_count(board):
    return sum(1 for r in range(HSV_ROWS) for c in HSV_COLS
               if board[r * 8 + c] != 0xFF and (board[r * 8 + c] & 0xF0) == 0xD0)
...
def eval_ref(board, w, fl):
    if HSV_ON:
        return _s16(_eval_ref_base(board, w, fl) - HSV_W * hsv_count(board))
    return _eval_ref_base(board, w, fl)


def _eval_ref_base(board, w, fl):
...
def gate_rtl(args):
    ...
    parse_path = args.rtl
    if True:          # always parse the PREPROCESSED text (the .sv may carry `ifdef blocks)
        os.makedirs(BUILD, exist_ok=True)
        parse_path = os.path.join(BUILD, "LeafEval_pp.sv")
        pp = subprocess.run(["verilator", "-E", "-P", "--pp-comments"] + ["-D" + d for d in defines] + [args.rtl],
                            capture_output=True, text=True)
        if pp.returncode != 0:
            print(pp.stderr[-2000:]); print("GATE FAIL rtl: preprocess failed"); return 1
        open(parse_path, "w").write(pp.stdout)
    p = rtlparse.parse_leafeval(parse_path)
    HSV_ON = bool(p["meta"].get("hsv_combine"))
    print("HSV reference: %s" % ("ON (spec-fixed: -%d per virus, rows < %d, cols %s)" % (HSV_W, HSV_ROWS, HSV_COLS)
                                 if HSV_ON else "off"))
    ...
    cmd = (["verilator", "--cc", "--exe", "--build", "-j", "2", "-O2", "-Wno-fatal"] + ["-D" + d for d in defines] + [
