#!/usr/bin/env bash
# Gates for the DRHSV timing FALLBACK (LeafEval `ifdef DRLEV_SQREG / DRLEV_WRREG), reproducible.
...
cd "$H/gate"
PIPE="--define DRLEV_SQREG --define DRLEV_WRREG"
...
for v in base:"" pipe:"-DDRLEV_SQREG -DDRLEV_WRREG"; do n=${v%%:*}; d=${v#*:}; mkdir -p "$T/ln_$n"
...
PY
for m in row col sign delta wrdata sqswap sqlag3 sqlag2; do
  $PY gate.py rtl --rtl "$T/pmut_$m/LeafEval.sv" --define DRHSV $PIPE --build "$T/gbp_$m" > "$T/gate_pmut_$m.log" 2>&1 &
done
mkdir -p "$T/pmut_wrlnk_pp"; verilator -E -P --pp-comments -DDRLEV_SQREG -DDRLEV_WRREG "$T/pmut_wrlnk/LeafEval.sv" > "$T/pmut_wrlnk_pp/LeafEval.sv"
...
for m in row col sign delta wrdata sqswap sqlag3 sqlag2; do echo "== mutant $m: $(command grep -a -E '^PHASE' "$T/gate_pmut_$m.log" | tr '\n' ' ')"; done
...
for cfg in hsv:"-DDRHSV" pipe:"-DDRHSV -DDRLEV_SQREG -DDRLEV_WRREG"; do n=${cfg%%:*}; d=${cfg#*:}
