...
// public_flat_rd exposes the array to the C++ testbench so the agreement rig can sample the
// result mailbox WITHOUT issuing host bus cycles. Read-only, and this file is the sim-only
// stand-in (see header), so it cannot reach hardware. The run proves it is non-perturbing by
// reproducing dist69's per-board DONE clock counts exactly.
