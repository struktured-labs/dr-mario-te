...
PROPH_MD5 = "30c921837fdaa8e915175cdcf8709a24"   # the DRPROPH=1 deliverable (gate 1)
os.environ["DRPROPH"] = "1"                      # leak-through: every arm builds PROPH
...
def h1():
    ...
    shipped = PROPH_MD5    # gate-4 PROPH arm: the deliverable is the object under test
    a, err = build(f"{TMP}/h1_rebuild.nes", {})
    check("H1a deterministic rebuild == the DRPROPH=1 deliverable", a is not None
...
def main():
    ...
    check("HP0 IR capture contains the PROPH code (proph_trigger/proph_pulse labels)",
          {"proph_trigger", "proph_pulse"} <= have, str(sorted(l for l in have if "proph" in l)))
