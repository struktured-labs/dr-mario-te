...
--
-- Env: P6_OUT P6_MAXF P6_DLAT P6_SEED P6_TAG P6_TUCK (1=publish descriptors, 0=never)
-- ============================================================================
-- ---- REQUIRED IDENTITY (added after an ORPHAN LOG incident; see probe8.lua) ------------
-- OUT and TAG used to default to "." and "probe5". A launch with no P6_* environment then
...
local OUT   = need("P6_OUT")
local TAG   = need("P6_TAG")
local CART, CARTID = "?", "?"
pcall(function()
...
local MAXF  = tonumber(os.getenv("P6_MAXF") or "18000")
local DLAT  = tonumber(os.getenv("P6_DLAT") or "34")
local SEED  = tonumber(os.getenv("P6_SEED") or "114")
local PUBT  = tonumber(os.getenv("P6_TUCK") or "1")     -- 0 => always publish 0xFF (no tuck)
local BOOTF = tonumber(os.getenv("P6_BOOTF") or "10")
local DWELL = tonumber(os.getenv("P6_DWELL") or "2")    -- D2: frames on the approach column
...
local logf = io.open(OUT .. "/probe6.log", "w")
...
-- SYNTHETIC EXECUTOR PROBE (P6_TUCK=2). Isolates the CART's capability from board luck: it
...
log(string.format("probe6 start cart=%s cartid=%s nonce=%s out=%s tag=%s maxf=%d dlat=%d seed=%d pubtuck=%d dwell=%d",
