...
local logf = io.open(OUT .. "/probe_soak_fixed.log", "w")
...
emu.addEventCallback(function()
  -- ⚠ THE ONE-LINE FIX UNDER TEST. modeCache is assigned in the endFrame callback, so during a
  -- frame it still holds the PREVIOUS frame's mode. On the frame the game enters mode 4 the guard
  -- therefore does not fire, and any press still live is delivered into frame 1 of the match --
  -- which on a DRHUMAN cart is START = PAUSE. That is what froze seed 30011 at f=37,471, where a
  -- press armed at f=37,470 (37,470 % 30 == 0) was live across f=37,470..37,473.
  -- Reading $46 LIVE closes the one-frame window. Everything else in this file is byte-identical
  -- to the frozen soak instrument 40699e4c, so this arm is a single-variable necessity test.
  local liveMode = emu.read(0x46, emu.memType.nesMemory, false)
  if inCur and frame < inUntil and modeCache ~= 4 and liveMode ~= 4 then emu.setInput(inCur, 0) end
...
log(string.format("probe_soak_fixed start tag=%s maxf=%d dlat=%d seed=%d pubtuck=%d dwell=%d ckpt=%d " ..
