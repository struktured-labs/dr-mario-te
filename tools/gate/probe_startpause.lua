...
local logf = io.open(OUT .. "/probe_startpause.log", "w")
...
emu.addEventCallback(function()
  -- INJECT MODE 3 (killed mutant for the seed-30011 freeze): deliver START *during* mode 4.
  -- The frozen soak instrument does this BY ACCIDENT on the single frame where modeCache still
  -- holds the pre-transition mode, which is exactly the condition that held at f=37,470..37,473
  -- when the 8->4 transition landed at f=37,471. If a DRHUMAN cart pauses and produces the same
  -- signature -- mode 4 held forever, no MATCH record, goes still advancing, framebuffer frozen
  -- -- then that freeze is the harness, not the cart. Deliberately bypasses the mode guard.
  -- fire ONCE, on the first mode-4 frame at or after INJA, then hold 4 frames -- the same
  -- duration the accidental press had. A fixed [INJA, INJA+3] window would usually miss,
  -- because whether mode is 4 at an arbitrary frame is luck.
  if K.INJ == 3 and modeCache == 4 and frame >= K.INJA and (K.fired == nil or frame <= K.fired + 3) then
    if K.fired == nil then K.fired = frame; log("STARTINJECT f=" .. frame .. " (pressing START during mode 4)") end
...
log(string.format("probe_startpause start tag=%s maxf=%d dlat=%d seed=%d pubtuck=%d dwell=%d ckpt=%d " ..
