...
arm () {  # $1 cart  $2 tag
  local out="$D/tmp/pswit/$2"; mkdir -p "$out"
  ...
  ( cd "$(dirname "$MESEN")"
    ...
    exec "$RUN_MESEN" "$mmc1" "$D/tools/gate/probe_prespipe.lua" -testrunner "-timeout=$((FR/12+300))"
  ) >"$out/stdout.log" 2>&1 &
  local pid=$!
  for _ in $(seq 1 $((FR/12+300))); do
    command grep -aq "SUMMARY tag=$2" "$out/prespipe.log" 2>/dev/null && break
