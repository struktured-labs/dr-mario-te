#!/bin/bash
# run_rotpc.sh <orient 0..3> <frames> -- one constant-orient arm of the #132 mechanism probe.
...
orient="${1:?orient 0..3}"; maxf="${2:?frames}"; seed="${RP_SEED:-114}"
tag="rp_o${orient}_${maxf}_s${seed}${RP_CTLFRAME:+_ctl$RP_CTLFRAME}"
...
out="$D/tmp/rotpc/$tag"; mkdir -p "$out"
...
mmc1="$out/${tag}_mmc1.nes"; log="$out/rotpc.log"
...
for try in 1 2 3; do
  ...
  (
    ...
    export RP_OUT="$out" RP_TAG="$tag" RP_W="$W" RP_ORIENT="$orient" RP_MAXF="$maxf" \
           RP_SEED="$seed" RP_DLAT="${RP_DLAT:-34}" RP_STALLN="${RP_STALLN:-300}" \
           RP_PCFRAMES="${RP_PCFRAMES:-40}" RP_CTLFRAME="${RP_CTLFRAME:-0}"
    exec "$RUN_MESEN" "$mmc1" "$D/tools/gate/probe_rotpc.lua" -testrunner "-timeout=$deadline"
  ) >"$out/stdout.log" 2>&1 &
  runpid=$!
