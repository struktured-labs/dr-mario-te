#!/bin/bash
# run_one8.sh <tag> <cart.nes> <maxf> [P8_TUCK] -- ONE probe5 arm, FRESH Xvfb, verified.
#
...
for try in 1 2 3; do
  ...
  ( cd /home/struktured/projects/dr-mario-mods/mesen2/bin/linux-x64/Release
    export DISPLAY=":$disp" P8_OUT="$out" P8_MAXF="$maxf" P8_TAG="$tag" P8_DLAT=34 P8_SEED=114 P8_TUCK="$pubt" P8_SLACK="${P8_SLACK:-2}"
    exec /home/struktured/projects/dr-mario-mods/run_mesen.sh "$mmc1" \
      "$D/tools/gate/probe8.lua" ) >"$out/stdout.log" 2>&1 &
  runpid=$!
  ok=0
  for _ in $(seq 1 $((deadline / 2))); do
    if command grep -aq "SUMMARY tag=$tag" "$out/probe8.log" 2>/dev/null; then ok=1; break; fi
    kill -0 "$runpid" 2>/dev/null || { sleep 3; \
      command grep -aq "SUMMARY tag=$tag" "$out/probe8.log" 2>/dev/null && ok=1; break; }
    sleep 2
  done
  ...
  sleep 2
  if [ "$ok" = 1 ]; then command grep -a "SUMMARY tag=$tag" "$out/probe8.log"; exit 0; fi
