...
local logf = io.open(OUT .. "/probe8.log", "w")
...
-- SYNTHETIC EXECUTOR PROBE (P8_TUCK=2). Isolates the CART's capability from board luck: it
...
-- ⚠ PRE-REGISTERED DOSE-RESPONSE (probe8). SLACK is now a knob, and the whole point of the
-- experiment. HYPOTHESIS: the executor's completion failures are NOT depth per se -- they are
-- REMAINING FALL BELOW THE SWITCH. The lateral move needs fall budget, and DRDISTGATE actively
-- clamps the target to the columns reachable in the fall that is left; at zero budget the
-- switch is a no-op and the capsule is STRANDED in the approach column, which is the
-- documented "strictly worse than no tuck".
-- PREDICTION, written before the runs: SLACK=0 stranded > 0 ; SLACK=4 stranded == 0.
-- If SLACK=0 completes cleanly, the hypothesis is REFUTED and I report that instead.
local SLACK = tonumber(os.getenv("P8_SLACK") or "2")
...
local function synth_tuck(bd, argcol)
  ...
  if rt < MINROW then return nil end   -- keep the descriptor deep enough to beat DLAT
