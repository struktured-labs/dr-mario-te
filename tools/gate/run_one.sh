...
for try in 1 2 3 4 5; do
  ...
  ( cd /home/struktured/projects/dr-mario-mods/mesen2/bin/linux-x64/Release
    export DISPLAY=":$disp" P3_OUT="$out" P3_MAXF="$maxf" P3_TAG="$tag" P3_DLAT=34 P3_SEED=114
    timeout -k 10 400 /home/struktured/projects/dr-mario-mods/run_mesen.sh "$cart" \
      "$D/tools/gate/probe3.lua" ) >"$out/stdout.log" 2>&1
  kill "$xpid" 2>/dev/null
  if command grep -aq "tag=$tag" "$out/probe3.log" 2>/dev/null; then
    command grep -a SUMMARY "$out/probe3.log"; exit 0
  fi
