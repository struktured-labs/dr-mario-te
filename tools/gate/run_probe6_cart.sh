...
W=${P6_W_OVERRIDE:-0x5200}

# #148 BLACK-BOX variant: takes a CART PATH plus its EXPECTED md5, because the cart under
# test (drmario_copro_human_nofreeze, 4b98d6c8) predates romgen and has no manifest anywhere
# in roms/manifests/. The md5 is still mandatory -- a pre-manifest cart is exactly the case
# where "identify by filename" has already burned us (dr-mario-base-rom-collision).
cart="${1:?cart path}"; want_md5="${2:?expected md5}"; maxf="${3:-18000}"
