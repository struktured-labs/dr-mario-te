#!/bin/bash
# launch_flip.sh <outdir> <maxframes> [rlat] [dlat] [flipf] [seed] [display] [timeout_s]
# Same as launch_census.sh but runs census_run_flip.lua (running-best flip arm).
set -u
CEN=/mnt/data/drmario/pocket-copro/mesen_copro_qa/census
OUTDIR="${1:?outdir}"; MAXF="${2:?maxframes}"
RLAT="${3:-2}"; DLAT="${4:-34}"; FLIPF="${5:-20}"; SEED="${6:-1}"
DISP="${7:-:158}"; TMO="${8:-260}"
...
CEN_FLIPF="$FLIPF" CEN_SHOTS=0 CEN_SEED="$SEED" \
timeout -k 10 "$TMO" /home/struktured/projects/dr-mario-mods/run_mesen.sh \
  "$CEN/v4_mmc1.nes" "$CEN/census_run_flip_shots.lua" > "$OUTDIR/mesen_stdout.log" 2>&1
RC=$?
[ "$STARTED_XVFB" = 1 ] && [ -n "$XPID" ] && kill "$XPID" 2>/dev/null
echo "launch_flip done rc=$RC out=$OUTDIR"
