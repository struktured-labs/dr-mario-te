-- ============================================================================
-- census_run_board.lua — census_run.lua + board128 hex column (GO-snapshot board)
-- for the offline eval-weirdness control (committed vs flag-matched python golden).
-- Identical to census_run.lua in all behavior; only adds a CSV column.
...
csv:write("seq,round,go_f,go_count,res_f,done_f,lock_f,cA,cB,ccol,co4,tgtC,tgtO,rotdone," ..
          ...
          "h_go,vc_go,seed17,seed18,newcells,flag,board128\n")
  ...
  for o4 = 0, 3 do
    ...
    for c = 0, maxc do
      ...
      if cells then
        ...
        if sc > bestsc then bestsc = sc; bestc = c; besto = o4 end
      end
    end
  end
  return bestc, besto
...
local S = { board = {}, go_pulse = false, pending = false, need_snap = false,
            ...
            rcol = 0xFF, ror = 0xFF, done = false, cA = 0, cB = 0, goes = 0, dones = 0 }
...
local function compute_if_due()
  ...
  if S.pending and not S.need_snap and S.ror == 0xFF
    ...
    local c, o = brain(S.board, S.cA, S.cB)
...
emu.addMemoryCallback(function(addr, value)  -- GO strobe (write $5084): pure Lua only
  ...
  S.done = false; S.rcol = 0xFF; S.ror = 0xFF
...
emu.addMemoryCallback(function(addr, value)  -- DONE read
  ...
  compute_if_due()
  if S.pending and S.ror ~= 0xFF and (curFrame - S.go_frame) >= DLAT then
...
emu.addMemoryCallback(function(addr, value)
  compute_if_due()
  if S.rcol == 0xFF then return 0 end
  return S.rcol
...
local function close_rec(lockf, cells, flag)
  seq = seq + 1
  csv:write(string.format("%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%s,%s,%s\n",
    ...
    rec.h_go, rec.vc_go, seed17, seed18, cells_str(cells), flag, rec.bhex or ""))
  csv:flush()
  if flag == "ok" and shotN < SHOTS then
    shotN = shotN + 1
    shot(string.format("lock_%03d_f%d.png", seq, lockf))
  end
  -- targeted mismatch screenshots: landed geometry vs committed (offline decode is
  -- authoritative; this is just a coarse trigger to save visual evidence at lock time)
  if flag == "ok" and rec.ccol >= 0 and #cells == 2 and mmShots < 24 then
    ...
    if kind ~= ckind or a[2] ~= rec.ccol then
      mmShots = mmShots + 1
      shot(string.format("mm_%s_seq%03d_c%d_o%d_land%s%d.png",
        (kind ~= ckind) and "orient" or "col", seq, rec.ccol, rec.co4, kind, a[2]))
    end
  end
  rec = nil
  lastActivity = frame
  ...
  local ok, err = pcall(function()
    ...
    if S.go_pulse then
      ...
      if rec and rec.cA == (rd(0x0381) % 16) and rec.cB == (rd(0x0382) % 16) then
        ...
        local bh = {}
        for i = 0, 127 do bh[i + 1] = string.format("%02X", rec.snap[i]) end
        rec.bhex = table.concat(bh)
      end
      lastActivity = frame
    end
    ...
    if rec then
      ...
      if rec.ccol == -1 and S.ror ~= 0xFF then
        rec.ccol, rec.co4, rec.res_f = S.rcol, S.ror, S.res_frame
      end
...
log(string.format("census_run_board loaded RLAT=%d DLAT=%d MAXF=%d SHOTS=%d SEED=%d out=%s", RLAT, DLAT, MAXF, SHOTS, SEED, OUT))
