...
csv:write("seq,round,go_f,go_count,res_f,done_f,lock_f,cA,cB,ccol,co4,tgtC,tgtO,rotdone," ..
          ...
          "h_go,vc_go,seed17,seed18,newcells,flag\n")
...
local function close_rec(lockf, cells, flag)
  seq = seq + 1
  csv:write(string.format("%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%s,%s\n",
...
log(string.format("census_run loaded RLAT=%d DLAT=%d MAXF=%d SHOTS=%d SEED=%d out=%s", RLAT, DLAT, MAXF, SHOTS, SEED, OUT))
