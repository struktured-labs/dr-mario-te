...
> **Canonical live memory:** `source/PROJECT_MEMORY.md` at commit `1d5e3cc`.
> This root copy is an older snapshot; resume from the canonical file, which
> includes the 2026-08-11 exogenous-pressure, oracle-teacher, Pocket and tuck
> findings.
...
- Per-flip provenance shipped on `flip-provenance` at `5312267`. The shared
  evaluator/oracle schema convergence is completed and gated in this workspace;
  it still needs committing from a workspace with writable Git metadata.
- Oracle work is on `oracle-ceiling` at `48cd4f6`. Identity, liveness, capsule
  independence, and fork-leak gates passed. Its pilot is `ORACLE-CLAIR` and is
  not a usable ceiling because it sees the realized future garbage columns.
  `ORACLE-DIST`, the intended headline arm, is preregistered but not yet
  implemented.
...
3. Implement and gate `ORACLE-DIST`; do not quote `ORACLE-CLAIR` as the
   programme ceiling.
...
The fork also sees future capsules, but the target cannot. The host bridge
exposes only current `cA/cB` and preview `nA/nB` at `$5080-$5083`; shipped d3
uses a small expectimax pill table rather than the realized future stream.
Therefore a DIST GO may still contain unactionable capsule clairvoyance. Report
or separate that channel just as realized garbage clairvoyance was separated.
...
1. Should a DIST NO_GO close only the fixed top-4/H15/gated probe, or do we want
   to pay for the K/horizon/action-coverage evidence needed to retain the
   stronger “all root re-ranking is dead” claim?
